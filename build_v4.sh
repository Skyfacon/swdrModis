...
install_package "oneTBB-2020.3" \
    ...
    "zxvf v2020.3.tar.gz" \
    "cd .. && make -j8 && BUILD_DIR=\$(find build -type d -name 'linux*release') && mkdir -p lib && cp \$BUILD_DIR/* lib/"
...
#     "../configure --prefix=/usr/local/sqlite3460000 --enable-rtree=yes && make && make install &&  cp /usr/local/sqlite3460000/include/* /usr/local/include/ && cp /usr/local/sqlite3460000/lib/* /usr/local/lib64/ && mv /usr/bin/sqlite3 /usr/bin/sqlite3_old && ln -fs /usr/local/sqlite3460000/bin/sqlite3 /usr/bin/sqlite3"

# Proj 安装
install_package "proj-9.4.0" \
    "https://download.osgeo.org/proj/proj-9.4.0.tar.gz" \
    "zxvf proj-9.4.0.tar.gz" \
    "cmake -D CMAKE_EXE_LINKER_FLAGS='-lpthread -pthread' -DSQLITE3_DIR=/usr/local/sqlite3460000/ .. && cmake --build . --parallel \$(nproc) && cmake --build . --target install"

# GDAL 安装
install_package "gdal-3.9.0" \
    "https://github.com/OSGeo/gdal/releases/download/v3.9.0/gdal-3.9.0.tar.gz" \
    "zxvf gdal-3.9.0.tar.gz" \
    "cmake .. -DCMAKE_BUILD_TYPE=Release -DCMAKE_EXE_LINKER_FLAGS='-lpthread -pthread' && cmake --build . --parallel \$(nproc) && cmake --build . --target install"
