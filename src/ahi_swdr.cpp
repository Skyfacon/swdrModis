...
int ahi_swdr::retrieve_image(const std::string& input_file, const std::string& out_file)
{
	...
	//timer.tic();
	//----------------------------------------
	for (uword i = dw_sza_idx_image; i < up_sza_idx_image; i++) //10度,最大值到85
	{
		uword up_sza_idx;
		uword dw_sza_idx;
		uword up_vza_idx;
		...
		for (uword j = dw_vza_idx_image; j < up_vza_idx_image; j++)
		{
			...
			for (uword m = 0; m < 5; m++)
			{
				...
				for (uword n = 0; n < 5; n++)
				{
					...
					if (idx_tile.n_elem < 5) continue; //改成小于5可以提升2秒左右
					...
					if (idx_nosnow_2.n_elem != 0)
					{
						...
						//多加一个LUT子表传递,!!可以再多加一个总表传递，如果一个角度条件下，在子表找不到可用变量时，可以用总表找可用变量
	...
	fluxes.slice(6) = conv_to<Mat<short>>::from(toa_up_flux_mat * 10); //toa上行辐射
	fluxes.slice(7) = conv_to<Mat<short>>::from(rho_mat * 10000);      //大气球面反照率，用于临近效应计算
	fluxes.slice(8) = conv_to<Mat<short>>::from(sw_alb_mat * 10000);  //地表宽波段反照率，用于临近效应计算
	...
	arma::fmat& toa_rad_band1_lut_cloudy, arma::fmat& toa_rad_band3_lut_cloudy, arma::fmat& toa_rad_band7_lut_cloudy)
{
	...
	//晴空和多云直接给到指定的index，比find更快，晴空是VIS=20，COD=0; 多云是VIS=20，COD=60
	//======================================================================================
	...
	uvec idx_cloudy = { 243,257,271 }; //FY-3D，加了晴空的vis
	//------------------------------------------
	...
	arma::fvec& derived_toa_up_flux, arma::fvec& derived_rho)
{
	...
	//---------------------------------------------
	//FY-3D,第1，2波段和第3,4波段时相反的(！！代码不变，变查找表即可）
	const fvec i0_band1 = lut.col(5); //band1
	const fvec rho_band1 = lut.col(6);
	const fvec complex_var_band1 = lut.col(7);
	...
	//当前子表的行数，包括up/dw SZA, up/dw VZA, up/dw DEM, 每一个子块对应一个晴空，一个多云值
	...
	//======================================================
	//查找表的雪指数值(积雪指数的LUT分块只能循环完成）给定3个flag，每个flag有不同的形式
	fmat toa_rad_ndsi_lut_tile = (toa_rad_band4_lut_tile / 593.84 - toa_rad_band6_lut_tile / 76.53) / (toa_rad_band4_lut_tile / 593.84 + toa_rad_band6_lut_tile / 76.53);//验证过

	//------------计算子LUT的SWDR & PAR UVA UVB TOA_albedo-------------------------
	//=======swdr=======
	...
	fmat toa_up_flux_tile = toa_albedo_tile % f_toa_dw_flux_tile; //需要确定是否是每col相乘
...
int ahi_swdr::read_lut(const std::string& lut_file, const arma::uword& lut_cols)
{
	...
	const size_t ncols = lut_cols;  //LUT的列数
	...
	//===============================================================
	//// calculate ncols,用于自动化计算LUT的列，但是给定值更快
	...
	arma::fvec& itp_uva, arma::fvec& itp_uvb, arma::fvec& itp_toa_up_flux, arma::fvec& itp_rho) const
{
	...
	//晴空toa_rad
	fmat toa_rad_band1_clear_dem1_mat = toa_rad_band1_lut_clear_tile.rows(span(idx_up_dem, idx_up_dem + idx_filter_dem - 1));//其实一个idx_up_dem对应的应该全是一个值
...
////------------------------------------------------------------
	...
	////===================================================================================================================================
	////========================================================================================================
	////可以改读取LUT的方式，SZA一遍，VZA读取SZA的，下面就像素计算
	for (uword i = 0; i < toa_rad_b3_sub_v.n_rows; i++)
	{
		...
		uvec idx_snow_dem1 = find(toa_rad_ndsi_dem1 <= (lut_diff_max + 5 * vstd_ndsi) && toa_rad_ndsi_dem1 >= (lut_diff_min - 5 * vstd_ndsi)); //积雪分块过滤
		//
		...
		//判断大气条件
		//0-不确定，1-晴空，2-60以下薄云，3-厚云
		//dem1
		uword clear_flag_dem1 = 0;//晴空
		//晴空
		...
		//----------------------------------------------------------------------
		//dem2,初步判断晴空多云=================================================
		uword clear_flag_dem2 = 0;//flag为1标记为晴空
		//晴空
		...
		uvec idx_dir; 
		//-----------------------------------------------
		//分不同的大气情况计算idx_total和idx_dir
		//-----------------------------------------------
		if (clear_flag_dem1 == 1) //晴空
		{
			...
			while (idx1_revise.n_elem < 2)
			{
				...
				{
					idx1_revise = find(COD_dem1_revise >= 0); //相当于不进行修正，就是idx1本身
					break;
				}
			...
			//idx_total = idx2; //ln+欧式,blue,交集，总辐射和蓝光波段去交集
			idx_total = idx1;
			idx_dir = idx1; //ln+欧式

		}
		else if (clear_flag_dem1 == 3)//厚云，
		{
			//形状，cos
			//计算修正值
			float toa_revise0 = mean(toa_rad_cloud);  //观测值均值	
		...
		{
			//计算修正值
			float toa_revise0 = mean(toa_rad);  //观测值均值	
			...
			{
				//计算修正值
				toa_revise0 = mean(toa_rad_cloud);  //观测值均值	
		...
		//========================================================================================================
		//结合天气的，结合cos和欧式距离的多波段算法
		if (clear_flag_dem2 == 1) //晴空
		{
			...
			while (idx1_revise.n_elem < 2)
			{
				...
				if (toa_avg_num_new > idx1.n_elem)
				{
					idx1_revise = find(COD_dem2_revise >= 0); //相当于不进行修正，就是idx1本身
					break;
				}
				idx1 = idx1.head(toa_avg_num_new);
			...
			//idx_total = idx2; //总辐射，ln+cos,blue,交集
			idx_total = idx1;
			idx_dir = idx1;//直射辐射，ln+cos
		}
		else if (clear_flag_dem2 == 3)//厚云
		{
			////形状
			////计算修正值
			float toa_revise0 = mean(toa_rad_cloud);  //观测值均值	
